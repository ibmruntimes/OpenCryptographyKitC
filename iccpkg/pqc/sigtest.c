...
static void fprintBstr(FILE *fp, const char *S, const unsigned char *A, size_t L)
{
   size_t i;
...
int SignatureEVP_gen(ICC_CTX *ctx, const char *nm, pkbuf *p_pkc, skbuf *p_skc, enum ed encdec)
{
   ...
   /* reconstruct keys from encoding */
   {
      ...
      { /*compare pubkey and decoded key */
...
int SignatureEVP_sign(ICC_CTX *ctx, sbuf *sig, const skbuf *p_skc, const unsigned char *msg, size_t msg_len, enum ed encdec, const char *hash)
{
   int rc;
   size_t siglen = 0;
...
int SignatureEVP_verify(ICC_CTX *ctx, const pkbuf *p_pkc, const unsigned char *msg, size_t msg_len, const sbuf *sig, enum ed encdec, const char *hash)
{
   int rc = -1;
...
int PQC_sign_test(ICC_CTX *ctx, const char *algname, const char *hash, size_t msg_len, int verbose, int encdec)
{
   FILE *fp_rsp = stdout;
   int ret_val;
...
static const char *to_SIGNATURE_ALGNAME(int k)
{
...
static void fcb(const char *a, int b, int c)
{
   printf("fcb:%s, %d, %d\n", a, b, c);
}

static void tcb(const char *val1, const char *val2)
{
   printf("Tcb:%s, %s \n", val1, val2);
}

static int OpenSSLError(ICC_CTX *ctx)
{
